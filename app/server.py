...
@app.route("/")
def hello_world():
    real_time = avahi_browse()
    return render_template('index.html', data=real_time, delay=delay)


@app.route("/update")
def update_data():
    real_time = avahi_browse()
    all_time = read_csv(path_all_time_data)
    data = {'realTimeData': real_time, 'allTimeData': all_time}
    return jsonify(data)
